...
void printHelp()
{
  cout << "analysisWrapper options are:" << endl
       ...
       << "\t --seed    - set rundom seed (default = 0)" << endl
...
int main(int argc, char* argv[])
{
  ...
  int channel(0),charge(0),flag(-1),seed(0);
  float xsec(1.0);
  for(int i=1;i<argc;i++){
    ...
    else if(arg.find("--seed")!=string::npos && i+1<argc)       { sscanf(argv[i+1],"%d",&seed); i++;}
  ...
  if(method=="ExclusiveZX::RunExclusiveZX") {
    ...
    RunExclusiveTop(in,out,0,0,normH,puH,era,debug,systVar,seed);
  }
