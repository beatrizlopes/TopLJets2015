...
#include "TopLJets2015/TopAnalysis/interface/utils.h"
#include "TopLJets2015/TopAnalysis/interface/analysisUtils.h"
...
#include "TopLJets2015/TopAnalysis/interface/KinematicReconstruction_LSroutines.h"
#include "TopLJets2015/TopAnalysis/interface/KinematicReconstructionSolution.h"


constexpr double TopMASS_tmp = 172.5;
...
void KinematicReconstruction::setRandomNumberSeeds(const TLorentzVector& lepton, const TLorentzVector& antiLepton, const TLorentzVector& jet1, const TLorentzVector& jet2)const
{
                                                                    ...
                                                                    const TLorentzVector& met)const
{
    ...
    //redundant condition, already asked for in ControlSelection
        ...
        for(std::vector<int>::const_iterator j_index = i_index+1; j_index != bjetIndices.end(); ++j_index) {
            for(const int leptonIndex : leptonIndices) {
                for(const int antiLeptonIndex : antiLeptonIndices){
                    const std::vector<KinematicReconstructionSolution> solutions(this->solutionsPerObjectCombination(leptonIndex, antiLeptonIndex, *i_index, *j_index, allLeptons, allJets, met, 2));
                    result.addSolutions(solutions);
                    const std::vector<KinematicReconstructionSolution> solutionsSwapped(this->solutionsPerObjectCombination(leptonIndex, antiLeptonIndex, *j_index, *i_index, allLeptons, allJets, met, 2));
	...
	for(const int leptonIndex : leptonIndices)
	  for(const int antiLeptonIndex : antiLeptonIndices){
	    const std::vector<KinematicReconstructionSolution> solutions(this->solutionsPerObjectCombination(leptonIndex, antiLeptonIndex, bjetIndex, nonBjetIndex, allLeptons, allJets, met, 1));
	    result.addSolutions(solutions);
	    const std::vector<KinematicReconstructionSolution> solutionsSwapped(this->solutionsPerObjectCombination(leptonIndex, antiLeptonIndex, nonBjetIndex, bjetIndex, allLeptons, allJets, met, 1));
	    result.addSolutions(solutionsSwapped);
	  }
	...
	for(const int leptonIndex : leptonIndices)
	  for(const int antiLeptonIndex : antiLeptonIndices){
	    const std::vector<KinematicReconstructionSolution> solutions(this->solutionsPerObjectCombination(leptonIndex, antiLeptonIndex, *i_index, *j_index, allLeptons, allJets, met, 0));
	    result.addSolutions(solutions);
	    const std::vector<KinematicReconstructionSolution> solutionsSwapped(this->solutionsPerObjectCombination(leptonIndex, antiLeptonIndex, *j_index, *i_index, allLeptons, allJets, met, 0));
	    result.addSolutions(solutionsSwapped);
	  }
                                                                                                    ...
                                                                                                    const int numberOfBtags)const
{
  ...
  const TLorentzVector& lepton = allLeptons.at(leptonIndex);
  const TLorentzVector& antiLepton = allLeptons.at(antiLeptonIndex);
  const TLorentzVector& jet1 = allJets.at(jetIndex1);
  const TLorentzVector& jet2 = allJets.at(jetIndex2);

  KinematicReconstruction_MeanSol meanSolution(TopMASS_tmp, h_wmass_->GetRandom(), h_wmass_->GetRandom());
  //KinematicReconstruction_MeanSol meanSolution(TopMASS_tmp, 80.4, 80.4);
  //Smear solutions with smearing factors x and f from histograms from root file such that:
  //top = (x*Px, x*Py, x*Pz, f*E)
  //antitop = ...
  //....
  //and get MeanSolution with "this->solutionSmearing(...)"
  ...
  if(hasSolution){
    ...
    TLorentzVector wp;
    TLorentzVector wm;
    // From the vector of solutions, get mean sol. by taking the mean of the LV elements
    // //MOD
    //meanSolution.meanSolution(top, antiTop, neutrino, antiNeutrino);
    meanSolution.meanSolution(top, antiTop, wm, wp, neutrino, antiNeutrino);
    ...
    //Use the following only to get the number of (smeared) solutions of the kinematic system
    ...
    //here top, antiTop, neutrino and antiNeutrino are retrieved from inside the solutionSmearing(..) function,
    //which in turn use the KinematicReconstruction_LSroutines. Add all the Ws to this and also to the solution def. below.
    const KinematicReconstructionSolution solution(&allLeptons, &allJets,
						   leptonIndex, antiLeptonIndex, jetIndex1, jetIndex2,
						   top, antiTop, wm, wp,
						   neutrino, antiNeutrino,
						   TopMASS_tmp, numberOfBtags, m_weight, isNoSmearSol);
    //solution.print();
...
/*
void KinematicReconstruction::kinReco(const LorentzVector::Type& leptonMinus, const LorentzVector::Type& leptonPlus, const LorentzVector::Vector* jets, const std::vector<double>* btags, const LorentzVector::Type* met)
{
...
const LorentzVector::Type& jet1 = new_jets.at(bjetIndex);
const LorentzVector::Type& jet2 = new_jets.at(antiBjetIndex);
                                               ...
                                               const TLorentzVector& met)const
{
  ...
  const TLorentzVector l_temp    = lepton;
  const TLorentzVector al_temp   = antiLepton;
  const TLorentzVector b_temp    = jet1;
  const TLorentzVector bbar_temp = jet2;
  const TLorentzVector met_temp  = met;
  ...
  for(int sm=0; sm<100; ++sm){
    ...
    //This is the important W mass smearing
	...
	// Take only the first solution and add it to meanSolution
	for(int i=0; i<=tp_sm.getNsol()*0; ++i){
	  ...
	  meanSolution.add(tp_sm.getTtSol()->at(i).top,tp_sm.getTtSol()->at(i).topbar,
			   tp_sm.getTtSol()->at(i).wm, tp_sm.getTtSol()->at(i).wp,
			   tp_sm.getTtSol()->at(i).neutrino,tp_sm.getTtSol()->at(i).neutrinobar,mbl_weight);
	}
      }
  }
...
void KinematicReconstruction::loadData()
{
  ...
  TString data_path1 = "";
  data_path1.Append("/eos/user/b/bribeiro/ttbarKinRecoInput/KinReco_input_2016_Run2BtoH_25ns_80X_v037_dilep_sel_25Sep2017_NLODY_FineEleID.root");
...
/*
void KinematicReconstruction::kinRecoMassLoop(const LorentzVector::Type& leptonMinus, const LorentzVector::Type& leptonPlus, const LorentzVector::Vector* jets, const std::vector<double>* btags, const TLorentzVector* met)
{
    //FIXME:Warning , hardcoded value of b-tag working point. 
    constexpr double btag_wp = 0.244;
    ...
    const TLorentzVector leptonPlus_tlv  = LorentzVector::convert(leptonPlus);
    const TLorentzVector leptonMinus_tlv = LorentzVector::convert(leptonMinus);
    const TLorentzVector met_tlv         = LorentzVector::convert(*met);

    std::vector<TLorentzVector> jets_tlv;
    for (const auto& jet : *jets) {
        jets_tlv.push_back(LorentzVector::convert(jet));
    }
...
const TLorentzVector met_temp=LorentzVector::convert(*met);
