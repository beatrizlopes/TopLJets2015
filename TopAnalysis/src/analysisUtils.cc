...
void common::TLorentzVectortod4(const TLorentzVector& lv, double* d)
{
    d[0] = lv.E();
    d[1] = lv.Px();
    d[2] = lv.Py();
...
std::vector<double> common::parametersLV(const std::vector<TLorentzVector>& v_lv, const common::LVParameter& parameter)
{
    std::vector<double> v_variable;
    for(const TLorentzVector& lv : v_lv){
...
void common::orderIndices(int& index1, int& index2, const std::vector<TLorentzVector>& v_lv, const common::LVParameter& parameter, const bool absoluteValue)
{
...
void common::orderIndices(std::vector<int>& v_index, const std::vector<TLorentzVector>& v_lv, const common::LVParameter& parameter, const bool absoluteValue)
{
    const std::vector<double> v_variable = parametersLV(v_lv, parameter);
...
void common::selectIndicesLeptonIso(std::vector<int>& v_index, const std::vector<TLorentzVector>& v_lvLep, const std::vector<TLorentzVector>& v_lvJet)
{
    std::vector<int> result;
    for(const int index : v_index){
        bool isReject = 0;
        for(const TLorentzVector& jet : v_lvJet){
            TLorentzVector jetTLV = jet;
            TLorentzVector lepTLV = v_lvLep.at(index);
...
void common::selectIndices(std::vector<int>& v_index, const std::vector<TLorentzVector>& v_lv, const common::LVParameter& parameter,
...
int common::extremumIndex(const std::vector<TLorentzVector>& v_lv, const common::LVParameter& parameter, const bool maximumValue)
{
    const std::vector<double> v_variable = parametersLV(v_lv, parameter);
...
void common::orderTLorentzVector(TLorentzVector& lv1, TLorentzVector& lv2, const TLorentzVector& inputLv1, const TLorentzVector& inputLv2, const common::LVParameter& parameter, const bool absoluteValue)
{
    double variable1;
    double variable2;
    if(parameter == LVpt){
